; ModuleID = 'slash/main.a.i.p.s.r.i.bc'
...
declare dso_local i32 @add_one(i32, i32) local_unnamed_addr #1

declare dso_local i32 @add_two(i32, i32) local_unnamed_addr #1

declare dso_local i32 @add_three(i32, i32) local_unnamed_addr #1
...
18:                                               ; preds = %14
  ...
  %23 = icmp eq i32 %22, 0
  br i1 %23, label %select.unfold, label %.exit

select.unfold:                                    ; preds = %18, %entry1, %14
  %.4.i.ph = phi i32 (i32, i32)* [ @add_two, %14 ], [ @add_one, %entry1 ], [ @add_three, %18 ]
  %24 = icmp eq i32 (i32, i32)* %.4.i.ph, @add_one
  br i1 %24, label %if.true.direct_targ, label %if.false.orig_indirect

if.true.direct_targ:                              ; preds = %select.unfold
  %25 = tail call i32 @"__occam_spec.add_one(0x5,0x2)"()
  br label %if.end.icp

if.false.orig_indirect:                           ; preds = %select.unfold
  %26 = icmp eq i32 (i32, i32)* %.4.i.ph, @add_three
  br i1 %26, label %if.true.direct_targ1, label %if.false.orig_indirect2

if.true.direct_targ1:                             ; preds = %if.false.orig_indirect
  %27 = tail call i32 @"__occam_spec.add_three(0x5,0x2)"()
  br label %if.end.icp3

if.false.orig_indirect2:                          ; preds = %if.false.orig_indirect
  %28 = tail call i32 @"__occam_spec.add_two(0x5,0x2)"()
  br label %if.end.icp3

if.end.icp3:                                      ; preds = %if.false.orig_indirect2, %if.true.direct_targ1
  %29 = phi i32 [ %28, %if.false.orig_indirect2 ], [ %27, %if.true.direct_targ1 ]
  br label %if.end.icp

if.end.icp:                                       ; preds = %if.end.icp3, %if.true.direct_targ
  %30 = phi i32 [ %29, %if.end.icp3 ], [ %25, %if.true.direct_targ ]
  br label %.exit

.exit:                                            ; preds = %18, %if.end.icp
  %.01.i = phi i32 [ %30, %if.end.icp ], [ 0, %18 ]
  %31 = tail call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([17 x i8], [17 x i8]* @.str.3, i64 0, i64 0), i32 %.01.i) #3
  ret i32 %.01.i
}
