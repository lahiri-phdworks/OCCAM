...
class GadgetStats(object):
    ...
    def __init__(self, original, variant):
        ...
        self.totalUniqueCountDiff = len(
            original.totalUniqueGadgets) - len(variant.totalUniqueGadgets)
        if len(original.totalUniqueGadgets) > 0:
            self.totalUniqueCountReduction = self.totalUniqueCountDiff / \
                len(original.totalUniqueGadgets)
        else:
        ...
        if len(original.ROPGadgets) > 0:
            self.ROPCountReduction = self.ROPCountDiff / \
                len(original.ROPGadgets)
        else:
            self.ROPCountReduction = 0

        self.JOPCountDiff = len(original.JOPGadgets) - len(variant.JOPGadgets)
        if len(original.JOPGadgets) > 0:
            self.JOPCountReduction = self.JOPCountDiff / \
                len(original.JOPGadgets)
        else:
            self.JOPCountReduction = 0

        self.COPCountDiff = len(original.COPGadgets) - len(variant.COPGadgets)
        if len(original.COPGadgets) > 0:
            self.COPCountReduction = self.COPCountDiff / \
                len(original.COPGadgets)
        else:
            self.COPCountReduction = 0

        self.SysCountDiff = len(original.SysGadgets) - len(variant.SysGadgets)
        if len(original.SysGadgets) > 0:
            self.SysCountReduction = self.SysCountDiff / \
                len(original.SysGadgets)
        else:
            self.SysCountReduction = 0

        self.JOPDispatchersCountDiff = len(
            original.JOPDispatchers) - len(variant.JOPDispatchers)
        if len(original.JOPDispatchers) > 0:
            self.JOPDispatchersCountReduction = self.JOPDispatchersCountDiff / \
                len(original.JOPDispatchers)
        else:
            self.JOPDispatchersCountReduction = 0

        self.JOPDataLoadersCountDiff = len(
            original.JOPDataLoaders) - len(variant.JOPDataLoaders)
        if len(original.JOPDataLoaders) > 0:
            self.JOPDataLoadersCountReduction = self.JOPDataLoadersCountDiff / \
                len(original.JOPDataLoaders)
        else:
            self.JOPDataLoadersCountReduction = 0

        self.JOPInitializersCountDiff = len(
            original.JOPInitializers) - len(variant.JOPInitializers)
        if len(original.JOPInitializers) > 0:
            self.JOPInitializersCountReduction = self.JOPInitializersCountDiff / \
                len(original.JOPInitializers)
        else:
            self.JOPInitializersCountReduction = 0

        self.JOPTrampolinesCountDiff = len(
            original.JOPTrampolines) - len(variant.JOPTrampolines)
        if len(original.JOPTrampolines) > 0:
            self.JOPTrampolinesCountReduction = self.JOPTrampolinesCountDiff / \
                len(original.JOPTrampolines)
        else:
            self.JOPTrampolinesCountReduction = 0

        self.COPDispatchersCountDiff = len(
            original.COPDispatchers) - len(variant.COPDispatchers)
        if len(original.COPDispatchers) > 0:
            self.COPDispatchersCountReduction = self.COPDispatchersCountDiff / \
                len(original.COPDispatchers)
        else:
            self.COPDispatchersCountReduction = 0

        self.COPDataLoadersCountDiff = len(
            original.COPDataLoaders) - len(variant.COPDataLoaders)
        if len(original.COPDataLoaders) > 0:
            self.COPDataLoadersCountReduction = self.COPDataLoadersCountDiff / \
                len(original.COPDataLoaders)
        else:
            self.COPDataLoadersCountReduction = 0

        self.COPInitializersCountDiff = len(
            original.COPInitializers) - len(variant.COPInitializers)
        if len(original.COPInitializers) > 0:
            self.COPInitializersCountReduction = self.COPInitializersCountDiff / \
                len(original.COPInitializers)
        else:
            self.COPInitializersCountReduction = 0

        self.COPStrongTrampolinesCountDiff = len(
            original.COPStrongTrampolines) - len(variant.COPStrongTrampolines)
        if len(original.COPStrongTrampolines) > 0:
            self.COPStrongTrampolinesCountReduction = self.COPStrongTrampolinesCountDiff / \
                len(original.COPStrongTrampolines)
        else:
            self.COPStrongTrampolinesCountReduction = 0

        self.COPIntrastackPivotsCountDiff = len(
            original.COPIntrastackPivots) - len(variant.COPIntrastackPivots)
        if len(original.COPIntrastackPivots) > 0:
            self.COPIntrastackPivotsCountReduction = self.COPIntrastackPivotsCountDiff / \
                len(original.COPIntrastackPivots)
        else:
        ...
        self.totalUniqueIntroductionRate = len(
            self.totalUniqueIntroducedSet) / len(variant.totalUniqueGadgets)
        ...
        if len(variantROPSet) > 0:
            self.ROPIntroductionRate = len(
                self.ROPIntroducedSet) / len(variantROPSet)
        else:
        ...
        if len(variantJOPSet) > 0:
            self.JOPIntroductionRate = len(
                self.JOPIntroducedSet) / len(variantJOPSet)
        else:
        ...
        if len(variantCOPSet) > 0:
            self.COPIntroductionRate = len(
                self.COPIntroducedSet) / len(variantCOPSet)
        else:
        ...
        if len(variantSysSet) > 0:
            self.SysIntroductionRate = len(
                self.SysIntroducedSet) / len(variantSysSet)
        else:
            self.SysIntroductionRate = 0
        self.SysIntroducedGadgets = variant.getGadgetsFromStrings(
            self.SysIntroducedSet, "SYS")

        originalJOPDispatchersSet = GadgetSet.getGadgetTypeSet(
            original.JOPDispatchers)
        variantJOPDispatchersSet = GadgetSet.getGadgetTypeSet(
        ...
        if len(variantJOPDispatchersSet) > 0:
            self.JOPDispatchersIntroductionRate = len(
                self.JOPDispatchersIntroducedSet) / len(variantJOPDispatchersSet)
        else:
            self.JOPDispatchersIntroductionRate = 0
        self.JOPDispatchersIntroducedGadgets = variant.getGadgetsFromStrings(
            self.JOPDispatchersIntroducedSet, "JOP")

        originalJOPDataLoadersSet = GadgetSet.getGadgetTypeSet(
            original.JOPDataLoaders)
        variantJOPDataLoadersSet = GadgetSet.getGadgetTypeSet(
            variant.JOPDataLoaders)
        ...
        if len(variantJOPDataLoadersSet) > 0:
            self.JOPDataLoadersIntroductionRate = len(
                self.JOPDataLoadersIntroducedSet) / len(variantJOPDataLoadersSet)
        else:
            self.JOPDataLoadersIntroductionRate = 0
        self.JOPDataLoadersIntroducedGadgets = variant.getGadgetsFromStrings(
            self.JOPDataLoadersIntroducedSet, "JOP")

        originalJOPInitializersSet = GadgetSet.getGadgetTypeSet(
            original.JOPInitializers)
        variantJOPInitializersSet = GadgetSet.getGadgetTypeSet(
            variant.JOPInitializers)
        ...
        if len(variantJOPInitializersSet) > 0:
            self.JOPInitializersIntroductionRate = len(
                self.JOPInitializersIntroducedSet) / len(variantJOPInitializersSet)
        else:
            self.JOPInitializersIntroductionRate = 0
        self.JOPInitializersIntroducedGadgets = variant.getGadgetsFromStrings(
            self.JOPInitializersIntroducedSet, "JOP")

        originalJOPTrampolinesSet = GadgetSet.getGadgetTypeSet(
            original.JOPTrampolines)
        variantJOPTrampolinesSet = GadgetSet.getGadgetTypeSet(
            variant.JOPTrampolines)
        ...
        if len(variantJOPTrampolinesSet) > 0:
            self.JOPTrampolinesIntroductionRate = len(
                self.JOPTrampolinesIntroducedSet) / len(variantJOPTrampolinesSet)
        else:
            self.JOPTrampolinesIntroductionRate = 0
        self.JOPTrampolinesIntroducedGadgets = variant.getGadgetsFromStrings(
            self.JOPTrampolinesIntroducedSet, "JOP")

        originalCOPDispatchersSet = GadgetSet.getGadgetTypeSet(
            original.COPDispatchers)
        variantCOPDispatchersSet = GadgetSet.getGadgetTypeSet(
            variant.COPDispatchers)
        ...
        if len(variantCOPDispatchersSet) > 0:
            self.COPDispatchersIntroductionRate = len(
                self.COPDispatchersIntroducedSet) / len(variantCOPDispatchersSet)
        else:
            self.COPDispatchersIntroductionRate = 0
        self.COPDispatchersIntroducedGadgets = variant.getGadgetsFromStrings(
            self.COPDispatchersIntroducedSet, "COP")

        originalCOPDataLoadersSet = GadgetSet.getGadgetTypeSet(
            original.COPDataLoaders)
        variantCOPDataLoadersSet = GadgetSet.getGadgetTypeSet(
            variant.COPDataLoaders)
        ...
        if len(variantCOPDataLoadersSet) > 0:
            self.COPDataLoadersIntroductionRate = len(
                self.COPDataLoadersIntroducedSet) / len(variantCOPDataLoadersSet)
        else:
            self.COPDataLoadersIntroductionRate = 0
        self.COPDataLoadersIntroducedGadgets = variant.getGadgetsFromStrings(
            self.COPDataLoadersIntroducedSet, "COP")

        originalCOPInitializersSet = GadgetSet.getGadgetTypeSet(
            original.COPInitializers)
        variantCOPInitializersSet = GadgetSet.getGadgetTypeSet(
        ...
        if len(variantCOPInitializersSet) > 0:
            self.COPInitializersIntroductionRate = len(
                self.COPInitializersIntroducedSet) / len(variantCOPInitializersSet)
        else:
            self.COPInitializersIntroductionRate = 0
        self.COPInitializersIntroducedGadgets = variant.getGadgetsFromStrings(
            self.COPInitializersIntroducedSet, "COP")

        originalCOPStrongTrampolinesSet = GadgetSet.getGadgetTypeSet(
            original.COPStrongTrampolines)
        variantCOPStrongTrampolinesSet = GadgetSet.getGadgetTypeSet(
            variant.COPStrongTrampolines)
        commonCOPStrongTrampolinesSet = originalCOPStrongTrampolinesSet & variantCOPStrongTrampolinesSet
        self.COPStrongTrampolinesIntroducedSet = variantCOPStrongTrampolinesSet - \
            commonCOPStrongTrampolinesSet
        if len(variantCOPStrongTrampolinesSet) > 0:
            self.COPStrongTrampolinesIntroductionRate = len(
                self.COPStrongTrampolinesIntroducedSet) / len(variantCOPStrongTrampolinesSet)
        else:
            self.COPStrongTrampolinesIntroductionRate = 0
        self.COPStrongTrampolinesIntroducedGadgets = variant.getGadgetsFromStrings(
            self.COPStrongTrampolinesIntroducedSet, "COP")

        originalCOPIntrastackPivotsSet = GadgetSet.getGadgetTypeSet(
            original.COPIntrastackPivots)
        variantCOPIntrastackPivotsSet = GadgetSet.getGadgetTypeSet(
            variant.COPIntrastackPivots)
        commonCOPIntrastackPivotsSet = originalCOPIntrastackPivotsSet & variantCOPIntrastackPivotsSet
        self.COPIntrastackPivotsIntroducedSet = variantCOPIntrastackPivotsSet - \
            commonCOPIntrastackPivotsSet
        if len(variantCOPIntrastackPivotsSet) > 0:
            self.COPIntrastackPivotsIntroductionRate = len(
                self.COPIntrastackPivotsIntroducedSet) / len(variantCOPIntrastackPivotsSet)
        else:
            self.COPIntrastackPivotsIntroductionRate = 0
        self.COPIntrastackPivotsIntroducedGadgets = variant.getGadgetsFromStrings(
            self.COPIntrastackPivotsIntroducedSet, "COP")
        ...
        commonGadgets = GadgetStats.findEqualGadgets(original.ROPGadgets, variant.ROPGadgets) +\
            ...
            GadgetStats.findEqualGadgets(
                original.SysGadgets, variant.SysGadgets)
        totalGadgets = len(variant.ROPGadgets) + \
            len(variant.JOPGadgets) + len(variant.SysGadgets)
    ...
    def printStats(self):
        ...
        print("Reduction Rate: " +
              rate_format.format(self.totalUniqueCountReduction))
        print("Introduced Count: " + str(len(self.totalUniqueIntroducedSet)))
        print("Introduction Rate: " +
              rate_format.format(self.totalUniqueIntroductionRate))
        print("======================================================================")
        print("ROP Gadgets:")
