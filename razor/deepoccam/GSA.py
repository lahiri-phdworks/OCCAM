...
def run_gsa(original, variants_dict, directory_name):
    ...
    for key in variants_dict.keys():
        ...
        print("Analyzing variant package [" +
              key + "] located at: " + filepath)
    ...
    file_lines = [
        "Package Variant,Total Gadgets,ROP Gadgets,JOP Gadgets,COP Gadgets\r"]
    ...
    for stat in stats:
        stat_counts = stat.variant.name + "," + str(len(stat.variant.totalUniqueGadgets)) + " (" + str(
            stat.totalUniqueCountDiff) + "; " + rate_format.format(stat.totalUniqueCountReduction) + "),"
        stat_counts = stat_counts + str(len(stat.variant.ROPGadgets)) + " (" + str(
            stat.ROPCountDiff) + "; " + rate_format.format(stat.ROPCountReduction) + "),"
        stat_counts = stat_counts + str(len(stat.variant.JOPGadgets)) + " (" + str(
            stat.JOPCountDiff) + "; " + rate_format.format(stat.JOPCountReduction) + "),"
        stat_counts = stat_counts + str(len(stat.variant.COPGadgets)) + " (" + str(
            stat.COPCountDiff) + "; " + rate_format.format(stat.COPCountReduction) + ")\r"
    ...
    orig_counts = original.name + "," + \
        str(len(original.totalUniqueGadgets)) + ", , ,"
    orig_counts = orig_counts + str(len(original.ROPGadgets)) + ", , ,"
    orig_counts = orig_counts + str(len(original.JOPGadgets)) + ", , ,"
    orig_counts = orig_counts + str(len(original.COPGadgets)) + "\r"
    file_lines.append(orig_counts)
    ...
    for stat in stats:
        stat_counts = stat.variant.name + "," + \
            str(len(stat.variant.totalUniqueGadgets)) + ","
        stat_counts = stat_counts + \
            str(len(stat.totalUniqueIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.totalUniqueIntroductionRate) + ","
        stat_counts = stat_counts + str(len(stat.variant.ROPGadgets)) + ","
        stat_counts = stat_counts + str(len(stat.ROPIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.ROPIntroductionRate) + ","
        stat_counts = stat_counts + str(len(stat.variant.JOPGadgets)) + ","
        stat_counts = stat_counts + str(len(stat.JOPIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.JOPIntroductionRate) + ","
        stat_counts = stat_counts + str(len(stat.variant.COPGadgets)) + ","
        stat_counts = stat_counts + str(len(stat.COPIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.COPIntroductionRate) + "\r"
        file_lines.append(stat_counts)
    try:
        file = open(directory_name +
                    "/Gadget_Introduction_Counts_Rate.csv", "w")
        file.writelines(file_lines)
        file.close()
    except OSError as osErr:
        print(osErr)
    ...
    for stat in stats:
        ...
        stat_counts = stat_counts + str(len(stat.variant.JOPTrampolines)) + " (" + str(
            stat.JOPTrampolinesCountDiff) + "; " + rate_format.format(stat.JOPTrampolinesCountReduction) + ")\r"
        file_lines.append(stat_counts)
    ...
    file_lines.append(
        "Package Variant,COP Dispatcher Gadgets,COP Dataloader Gadgets,COP Initializers,COP Strong Trampoline Gadgets,COP Intra-stack Pivot Gadgets\r")
    ...
    orig_counts = orig_counts + "," + \
        str(len(original.COPIntrastackPivots)) + "\r"
    file_lines.append(orig_counts)

    for stat in stats:
        stat_counts = stat.variant.name + "," + str(len(stat.variant.COPDispatchers)) + " (" + str(
        ...
        stat_counts = stat_counts + str(len(stat.variant.COPStrongTrampolines)) + " (" + str(
            stat.COPStrongTrampolinesCountDiff) + "; " + rate_format.format(stat.COPStrongTrampolinesCountReduction) + "),"
        stat_counts = stat_counts + str(len(stat.variant.COPIntrastackPivots)) + " (" + str(
            stat.COPIntrastackPivotsCountDiff) + "; " + rate_format.format(stat.COPIntrastackPivotsCountReduction) + ")\r"
        file_lines.append(stat_counts)

    try:
        file = open(directory_name +
                    "/SpecialPurpose_GadgetCounts_Reduction.csv", "w")
        file.writelines(file_lines)
        file.close()
    except OSError as osErr:
        print(osErr)
    ...
    for stat in stats:
        stat_counts = stat.variant.name + "," + \
            str(len(stat.variant.SysGadgets)) + ","
        stat_counts = stat_counts + str(len(stat.SysIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.SysIntroductionRate) + ","
        stat_counts = stat_counts + str(len(stat.variant.JOPDispatchers)) + ","
        stat_counts = stat_counts + \
            str(len(stat.JOPDispatchersIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.JOPDispatchersIntroductionRate) + ","
        stat_counts = stat_counts + str(len(stat.variant.JOPDataLoaders)) + ","
        stat_counts = stat_counts + \
            str(len(stat.JOPDataLoadersIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.JOPDataLoadersIntroductionRate) + ","
        stat_counts = stat_counts + \
            str(len(stat.variant.JOPInitializers)) + ","
        stat_counts = stat_counts + \
            str(len(stat.JOPInitializersIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.JOPInitializersIntroductionRate) + ","
        stat_counts = stat_counts + str(len(stat.variant.JOPTrampolines)) + ","
        stat_counts = stat_counts + \
            str(len(stat.JOPTrampolinesIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.JOPTrampolinesIntroductionRate) + "\r"
        file_lines.append(stat_counts)
    ...
    orig_counts = original.name + "," + \
        str(len(original.COPDispatchers)) + ", , ,"
    orig_counts = orig_counts + str(len(original.COPDataLoaders)) + ", , ,"
    orig_counts = orig_counts + str(len(original.COPInitializers)) + ", , ,"
    orig_counts = orig_counts + \
        str(len(original.COPStrongTrampolines)) + ", , ,"
    orig_counts = orig_counts + str(len(original.COPIntrastackPivots)) + "\r"
    file_lines.append(orig_counts)

    for stat in stats:
        stat_counts = stat.variant.name + "," + \
            str(len(stat.variant.COPDispatchers)) + ","
        stat_counts = stat_counts + \
            str(len(stat.COPDispatchersIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.COPDispatchersIntroductionRate) + ","
        stat_counts = stat_counts + str(len(stat.variant.COPDataLoaders)) + ","
        stat_counts = stat_counts + \
            str(len(stat.COPDataLoadersIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.COPDataLoadersIntroductionRate) + ","
        stat_counts = stat_counts + \
            str(len(stat.variant.COPInitializers)) + ","
        stat_counts = stat_counts + \
            str(len(stat.COPInitializersIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.COPInitializersIntroductionRate) + ","
        stat_counts = stat_counts + \
            str(len(stat.variant.COPStrongTrampolines)) + ","
        stat_counts = stat_counts + \
            str(len(stat.COPStrongTrampolinesIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.COPStrongTrampolinesIntroductionRate) + ","
        stat_counts = stat_counts + \
            str(len(stat.variant.COPIntrastackPivots)) + ","
        stat_counts = stat_counts + \
            str(len(stat.COPIntrastackPivotsIntroducedSet)) + ","
        stat_counts = stat_counts + \
            rate_format.format(stat.COPIntrastackPivotsIntroductionRate) + "\r"
        file_lines.append(stat_counts)

    try:
        file = open(directory_name +
                    "/SpecialPurpose_Gadget_Introduction_Counts_Rate.csv", "w")
        file.writelines(file_lines)
        file.close()
    except OSError as osErr:
        print(osErr)
    ...
    for stat in stats:
        ...
        stat_counts = stat.variant.name + "," + \
            str(stat_simple_tc) + \
            " (" + str(stat.simpleTuringCompleteCountDiff) + ")\r"
        file_lines.append(stat_counts)
    ...
    for stat in stats:
        stat_locality = stat.variant.name + "," + \
            rate_format.format(stat.gadgetLocality) + "\r"
    ...
    for stat in stats:
        file_lines.append(
            "Sensitive gadgets introduced in variant: " + stat.variant.name + "\r")
        ...
        for specialSet in specialSets:
            for gadget in specialSet:
                ...
                if function is None:
                    ...
                    file_lines.append(
                        "Most likely location in source code: " + function + "\r")
        file_lines.append(
            "----------------------------------------------------------\r")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', default=os.path.join(OCCAM_HOME,
                                                   "examples/portfolio/tree"), help='work_dir')
