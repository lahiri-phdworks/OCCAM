...
def plot(no_of_sampling, no_of_run, workdir, graph_file, agg, opt,  metric="Total unique gadgets", ext="pdf"):
    ...
    def read_profiler(iteration, run, metric):
        res_path = os.path.join(
            workdir, "slash_run%s/run%s/0AfterSpecialization_results.txt" % (str(iteration), str(run)))
    ...
    def read_rop_profiler(iteration, run, metric):
        res_path = os.path.join(
            workdir, "slash_run%s/run%s/rop_stats.txt" % (str(iteration), str(run)))
        res = open(res_path, "r").readlines()
    ...
    def read_rop_stats(iteration, run, metric):
        res_path = os.path.join(
            workdir, "slash_run%s/run%s/gadget_count.json" % (str(iteration), str(run)))
    ...
    # save the summary
    summary = {"iters": iters, "means": means,
               "stdevs": stdevs, "agg": agg, "opt": opt}
...
class Dataset(object):
        ...
        for fname in csv_files:
            ...
            if self.collect_encoded_state and len(f_data) != len(f_enc_data):
                if DEBUG:
                    ...
                    print("there are only %s lines in .encoded_state" %
                          str(len(f_enc_data)))
                return
    ...
    def collect(self, size):
        ...
        for r in runs[:size]:
            ...
            self.no_good_runs += 1
        print("collected %s good runs out of %s runs" %
              (str(self.no_good_runs), str(size)))
        ...
        for eps in self.all_data:
            for i in range(len(eps["episode_data"])):
                ...
                for j in range(len(sub_episode)):
                    ...
                    action = torch.tensor(
                        step.action, dtype=torch.long).view(1, 1)
                    ...
                    reward = torch.tensor(
                        [self.get_step_reward(state, next_state, eps["score"])])
                    # if reward!=0:
...
def gen_new_meta(workdir, bootstrap_runs, run_command, get_rop_detail=False):
    metadata = {}
    # hard coded. = no of entry in inst2vec vocab
    metadata["padding_idx"] = 8565
    # hardcoded. assuming the longest len of a function is 2000
    metadata["max_sequence_len"] = 2000
    # hardcoded. assuming the longest len of args is 100
    metadata["max_args_len"] = 100
    ...
    if os.path.exists(dataset_path):
        ...
        clear_prev_runs = subprocess.check_output(
            ("rm -rf %s" % dataset_path).split())
    # run with policy none
    run_none = subprocess.check_output(
        "./build.sh -intra-spec none -folder none".split(), cwd=workdir)
    # run with policy aggressive
    run_aggr = subprocess.check_output(
        "./build.sh -intra-spec nonrec-aggressive -folder agg".split(), cwd=workdir)
    # build 1 2 3 4 ... k
    job_ids = ""
    for jid in range(bootstrap_runs):
        job_ids += " %s" % str(jid)
    # run the jobs
    runners_cmd = "parallel %s -epsilon 10.5 -folder {} 2>/dev/null ::: %s" % (
        run_command, job_ids)
    print(runners_cmd)
    try:
    ...
    if get_rop_detail:
        ...
        variants_dict = {"agg": os.path.join(
            workdir, "slash/runagg/%s" % binary_name)}
        for i in range(bootstrap_runs):
            variants_dict[str(i)] = os.path.join(
                workdir, "slash/run%s/%s" % (str(i), binary_name))
    ...
    def encode(self, insts):
        ...
        for inst in new_insts:
            ...
            for operand in tokens[1:]:
                ...
                encoded_str += "-".join([str(idx)
                                         for idx in token_symbols_idx])
                encoded_str += " "
            encoded_str += "\n"
        return encoded_str
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', default=100,
                        help='number of trial runs to get meta data')
