...
class GadgetSet(object):
    ...
    def __init__(self, name, filepath, createCFG):
        ...
        if createCFG:
            try:
                proj = angr.Project(filepath, main_opts={'base_addr': 0}, load_options={
        ...
        self.ROPGadgets = self.parseGadgets(
            "ROP", self.runROPgadget(filepath, "--nojop --nosys"))
        self.JOPGadgets = self.parseGadgets(
            "JOP", self.runROPgadget(filepath, "--norop --nosys"))
        self.SysGadgets = self.parseGadgets(
            "Syscall", self.runROPgadget(filepath, "--norop --nojop"))
        ...
        self.simpleTuringCompleteClasses = self.parseClasses(
            self.runROPgadget(filepath, "--nojop --nosys --microgadgets"))
    ...
    def runROPgadget(self, filepath, flags):
        ...
        bytestr = None
        try:
            bytestr = subprocess.check_output(
                "ROPgadget --binary " + filepath + " " + flags, shell=True)
    ...
    def populateSpecialJOPGadgets(self):
        ...
        for gadget in self.JOPGadgets:
            ...
            if last_instruction.find("[") > -1:
                ...
                base_target = last_instruction[base_target_start +
                                               2:base_target_start + 4]
                targets = ["r" + base_target, "e" + base_target]
                valid_opcodes = ["inc ", "dec ",
                                 "add ", "adc ", "sub ", "sbb "]
                target_operations = []
    ...
    def populateSpecialCOPGadgets(self):
        ...
        for gadget in self.COPGadgets:
            first_instruction = gadget.instructions[0]
            last_instruction = gadget.instructions[len(
                gadget.instructions) - 1]
            ...
            if first_instruction.find("pop ") > -1 and last_instruction.find("[") > -1:
                ...
                call_target = last_instruction[last_instruction.find(
                    "[")+1:last_instruction.find("]")]
                is_STG = False
                reject = False
