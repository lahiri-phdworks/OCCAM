...
class Net(nn.Module):
    def __init__(self, metadata):
        ...
        self.mean = torch.tensor(
            metadata["mean"], dtype=torch.float).view(1, -1)
...
class FeedForwardSingleInput(Net):
    def __init__(self, metadata):
        ...
        self.fc_f1 = nn.Linear(self.features_len, self.features_len, bias=True)
        self.fc_f2 = nn.Linear(
            self.features_len, self.features_len/2, bias=True)
        self.fc_f3 = nn.Linear(self.features_len/2,
                               self.features_len/4, bias=True)
    ...
    def forward(self, x):
        ...
        features = x - self.mean
        if DEBUG:
            print("x after subtract mean:", features)
        features /= self.std
        #print("x after normalization:", x)
        #features = x.to(torch.float)
        if DEBUG:
...
class FeedForwardSingleInputSoftmax(Net):
    def __init__(self, metadata):
        ...
        self.fc_f2 = nn.Linear(int(self.features_len),
                               int(self.features_len)/2)
...
class UberNet(Net):
    def __init__(self, metadata, dim_hidden=64, dim_hidden_args=16, dim_emb_args=4, num_layers=1):
        ...
        # layers
        self.gru_caller = nn.GRU(
            dim_emb, dim_hidden, num_layers, batch_first=True)
        self.gru_callee = nn.GRU(
            dim_emb, dim_hidden, num_layers, batch_first=True)
        self.gru_ctx = nn.GRU(dim_emb, dim_hidden,
                              num_layers, batch_first=True)
        self.gru_args = nn.GRU(
            dim_emb_args, dim_hidden_args, 1, batch_first=True)
        self.fc1 = nn.Linear(self.dim_hidden + self.dim_hidden +
                             self.dim_hidden + self.dim_hidden_args + len(self.extra_feat), 128)
    ...
    def forward(self, x):
        ...
        if DEBUG:
            print("caller_usage:", caller_usage)
        ...
        if DEBUG:
            print("ctx:", ctx)
        packed_caller = nn.utils.rnn.pack_padded_sequence(self.embedding(
            caller), caller_len, batch_first=True, enforce_sorted=False)
        packed_callee = nn.utils.rnn.pack_padded_sequence(self.embedding(
            callee), callee_len, batch_first=True, enforce_sorted=False)
        packed_args = nn.utils.rnn.pack_padded_sequence(self.embedding_args(
            args), args_len, batch_first=True, enforce_sorted=False)
        ...
        concat = torch.cat((last_h_caller[-1], last_h_callee[-1],
                            last_h_args[-1], last_h_ctx[-1], caller_usage.view(-1, 1)), -1)
        if DEBUG:
            print("concat:", concat)
