...
class QueryOracleServicer(Previrt_pb2_grpc.QueryOracleServicer):
    ...
    def __init__(self, mode, workdir, idx, metric, debug=True):
        ...
        if self.debug:
            print("read Meta from metadata.json...")
        self._got_step = Event()
        self._got_new_obs = Event()
        if self.debug:
            print("create Event object...")
        self._latest_obs = [-1]
        self.idx = idx
...
parser.add_argument('-w', '--workdir', default=os.path.join(OCCAM_HOME,
                                                            "examples/portfolio/tree/"), help='s')
