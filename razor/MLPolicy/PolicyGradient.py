...
class PolicyGradient(BasePolicy):
    def __init__(self, workdir, model_path, network_type, network_hp, grpc_mode, debug, metric):
        BasePolicy.__init__(self, workdir, model_path,
                            network_type, network_hp, grpc_mode, debug, metric)
        ...
        for i in range(no_of_iter):
            ...
            if self.grpc_mode:
                if self.net.net_type == "UberNet":
                    ...
                    workers = serve_multiple(
                        15, Mode.TRAINING_RNN, 1, 1, self.workdir, self.net)
                    # make sure all models and dicts are loaded into grpc_server
                    time.sleep(2)
                    ...
                    workers = serve_multiple(
                        20, Mode.TRAINING, 1, 1, self.workdir, self.net)
                    # make sure all models and dicts are loaded into grpc_server
                    time.sleep(1)
                    self.run_policy(no_of_sampling, eps_threshold, i)
                    time.sleep(1)  # make sure all clients have finished
...
#                server.stop(0)
            ...
            print("Rollout %s runs in %s seconds" %
                  (no_of_sampling, run_policy_time - start_time))
            dataset = Dataset(
                self.dataset_path, collect_encoded_state=collect_encoded_state, size=no_of_sampling)
            # abort bad runs
            if dataset.no_good_runs < MIN_USABLE_RUNS:
                print("too many broken runs")
                continue
            trajectory_data = dataset.get_trajectory_data(
                metric=self.metadata["metric"], normalize_rewards=self.normalize_rewards, baseline=self.metadata["none_results"][self.metadata["metric"]])
    ...
    def optimize(self, trajectory_data, iteration):
        ...
        print("state_tensor:", state_tensor.shape, "reward_tensor:",
              reward_tensor.shape, "action_tensor:", action_tensor.shape)
        ...
        if self.debug:
            print("state_tensor:", state_tensor[:debug_print_limit])
        if self.debug:
            print("predict_tensor:", predict_tensor[:debug_print_limit])
        #if self.debug: print("logprob:", logprob)
        if self.debug:
            print("reward_tensor", reward_tensor[:debug_print_limit])
        if self.debug:
            print("action_tensor", action_tensor[:debug_print_limit])
        ...
        if self.debug:
            print("selected_logprobs", selected_logprobs[:debug_print_limit])
        if minimize:
    ...
    def plot(self, no_of_sampling, iteration, fig_name):
        ...
        plot(no_of_sampling, iteration, self.workdir, fig_name,
             self.metadata["agg_results"][metric], self.metadata["none_results"][metric], self.metadata["metric"])
    ...
    def evaluate(self, tag="eval"):
        if self.grpc_mode:
            ...
            _ = subprocess.check_output(
                ("%s -epsilon 0 -folder %s 2>%s.log" % (self.run_command, tag, tag)).split(), cwd=self.workdir)
            server.stop(0)
        else:
            _ = subprocess.check_output(
                ("%s -epsilon 0 -folder %s 2>%s.log" % (self.run_command, tag, tag)).split(), cwd=self.workdir)
