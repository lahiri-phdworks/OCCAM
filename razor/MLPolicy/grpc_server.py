...
class QueryOracleServicer(Previrt_pb2_grpc.QueryOracleServicer):
    ...
    def __init__(self, mode, workdir, atomizer=None, net=None, debug=False, p=-1, n=-1):
        ...
        self.debug = debug
        if self.debug:
            print("init QueryOracleServicer...")
        self.module_trace = {}
        self.mode = mode
        self.say_no = False
        self.say_yes = False
        ...
        self.rewriter = TU.IRTransformer(os.path.join(
            OCCAM_HOME, "razor/MLPolicy/inst2vec/published_results/data/vocabulary"), self.meta["struct_dict"])
    ...
    def print_state(self, request):
        ...
        stmt_index, rewritten_ir = self.rewriter.llvm_ir_to_input(
    ...
    def Query(self, request, context):
        ...
        if self.mode == Mode.INTERACTIVE:
            ...
            pred = raw_input(
                "Should I specialize? ([y]es/[n]o/[Y]es for this callsite and the rest/[N]o for this callsite and the rest)")
            if pred.strip() == "y":
                pred = True
            elif pred.strip() == "Y":
                pred = True
        ...
        #context.set_trailing_metadata(('metadata_for_testint', b'I agree'),)
            ...
            args = request.args
            stmt_index, rewritten_ir = self.rewriter.llvm_ir_to_input(
                [caller, callee, ctx], ["caller", "callee", "ctx"])
            rewritten_caller = rewritten_ir[0]
            rewritten_callee = rewritten_ir[1]
            ...
            encoded_caller.extend(
                [0]*(self.meta["max_sequence_len"] - len(stmt_index[0])))

            encoded_callee = stmt_index[1]
            len_callee = len(encoded_callee)
            encoded_callee.extend(
                [0]*(self.meta["max_sequence_len"] - len(stmt_index[1])))
            ...
            if self.debug:
                print(state_encoded)
            features = [len_caller, len_callee, len_args, caller_usage] + \
                encoded_caller + encoded_callee + encoded_args + encoded_ctx    #
            features = torch.FloatTensor([features])
            if self.debug:
                print(features.shape)
            # #print(self.net)                                              #
...
def try_1_cs(p):
    ...
    server.add_insecure_port('[::]:50051')
    server.start()
    _ = subprocess.check_output(("./build.sh -g -epsilon 0 -folder %s" %
                                 str(p)).split(), cwd="/home/workspace/OCCAM/examples/portfolio/tree")
    server.stop(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', default=10, type=int,
                        help='position to specialize')
    parser.add_argument('-n', default=3, type=int, help='module to specialize')
    parser.add_argument('-mode', default='interactive', type=str,
                        help='grpc mode: training, interactive, try_1_cs')
    parser.add_argument(
        '-workdir', default='/home/workspace/OCCAM/examples/portfolio/tree')
